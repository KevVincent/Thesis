...
class Prediction(object):
	...
	def __init__(self, weights_path, backbone = 'inceptionresnet_v2',  encoder_freeze=False, n_classes=4):
		...
		self.preprocess_fn = sm.get_preprocessing(BACKBONE)
		self.activation = 'sigmoid' if self.n_classes == 1 else 'softmax'
		self.model = sm.Unet(self.backbone, classes=self.n_classes, activation=self.activation, encoder_freeze)
